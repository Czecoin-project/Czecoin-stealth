    ...
    model(0)
{
    ...
    ui->payTo->setPlaceholderText(tr("Enter a Czecoin address (e.g. CzecoinfwYhBmGXcFP2Po1NpRUEiK8km2)"));
...
//  if (fTestNet || pindexBest->nHeight > 9999) {
//	ui->narration->setPlaceholderText(tr("Enter a short note to send with payment (max 24 characters)"));
//	ui->narration->setEnabled(true);	}
//	if (!fTestNet && (pindexBest->nHeight < 10000)) {
//	ui->narration->setPlaceholderText(tr("Wait for block 10 000 to use narration")); 
//	ui->narration->setEnabled(false); }